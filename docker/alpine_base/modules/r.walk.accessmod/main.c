 ...
 *                 Markus Metz
 *               ** Accessmod 5 (am5) **
 *                - F.Moser (2014, 2020) Added custom cost map manager instead of Langmuir formula
...
int main(int argc, char *argv[])
{
    ...
    int speedMap; /* am5 using cell speed map  */ 
    int returnPath; /* am5 from or toward path  */
    ...
    */
    flag7 = G_define_flag();
    flag7->key = 's';
    flag7->description = _("Use speed by cells map (km/h) instead of friction map");
    /**
    * am5 slope diretion
    */
    flag8 = G_define_flag();
    flag8->key = 't';
    flag8->description = _("Inverse slope : cost toward point instead of from point");
    ...
    speedMap = flag7-> answer; /* am5 */
    returnPath = flag8->answer; /* am5 */
	...
	for (neighbor = 1; neighbor <= total_reviewed; neighbor++) {
		...
		G_fatal_error(_("Can not read from temporary file"));
            ...
            /*
            * am5 : added am5 "speed" map condition and cost manager
            */
            switch (neighbor) {
              case 1:
                W_dtm = costs.dtm;
