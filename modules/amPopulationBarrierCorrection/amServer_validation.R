...
observeEvent(listen$dataListUpdated,
  ...
  suspended = TRUE
) %>% amStoreObs(idModule, "update_pop_cor_input")
...
observe(
  {
    amErrorAction(title = "validation pop correct", {
      ...
      hasTags <- isNotEmpty(input$txtPopCorTags)
      ...
      output$uiPopCorValidation <- renderUI({
        msgList
      })
    })
  },
  suspended = TRUE
) %>% amStoreObs(idModule, "update_pop_cor_validation")
